...
class StoreTicketRequest extends FormRequest
{
    ...
    {
        return [
            'concern_type'              => ['string', 'required', 'in:gpo_app_service,reports,webtool'],
            'service_type'              => ['string', Rule::requiredIf(request()->get('concern_type') === 'gpo_app_service' && !request()->get('other_service')), 'nullable'],
            'other_service'             => ['string', Rule::requiredIf(request()->get('concern_type') === 'gpo_app_service' && !request()->get('service_type')), 'nullable'],
            'issue_details'             => ['required', 'string'],
            'customer_mobile_number'    => ['string', 'nullable'],
            'gpo_mobile_number'         => ['string', 'required_if:concern_type,gpo_app_service', 'nullable'],
            'device_model'              => ['string', Rule::requiredIf(request()->get('concern_type') === 'gpo_onboarding' || request()->get('concern_type') === 'gpo_connection'), 'nullable'],
            ...
            'transaction_amount'        => ['numeric', Rule::requiredIf(request()->get('concern_type') === 'gpo_app_service' && !!request()->get('service_type')), 'nullable'],
            'transaction_datetime'      => ['required_if:concern_type,gpo_app_service', 'date_format:Y-m-d H:i', 'nullable'],

            'portal_type'               => ['string', 'required_if:concern_type,webtool', 'nullable'],
            'webtool_role'              => ['string', 'nullable'],
